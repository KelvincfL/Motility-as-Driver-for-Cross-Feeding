...
def fitness(index:tuple,lattice, baseRate, cost, good_per_cooperator):
    ...
    return fitness_formula(lattice[index[0]][index[1]], neighbours[1], neighbours[0], baseRate, cost, good_per_cooperator)

### Reaction: Selection or Reproduction
def reaction_single(lattice, selectionRate, reproductionRate, colonisationRate, baseRate, deathRate, cost, good_per_cooperator):
    L = len(lattice[0])
    ...
    totalRate = selectionRate + reproductionRate + colonisationRate + deathRate

    #Reproduction if we have compatible pair (i.e. first cell empty)
    if reaction_type < reproductionRate / totalRate:
    ...
    elif reaction_type < (reproductionRate + deathRate)/totalRate:
        lattice[site_to_update[0]][site_to_update[1]] = 2 #death
    #Selection if we have compatible pair (i.e. first cell has lower fitness) - can kill or colonise
        ...
        relative_fitness = fitness(neighbour_index,lattice,baseRate,cost,good_per_cooperator) \
            - fitness(site_to_update,lattice,baseRate,cost,good_per_cooperator) #of neighbour vs update cell
        
        if relative_fitness > 0 or (relative_fitness == 0 and random.random() < 0.5): #greater fitness wins or equal prob
...
def simulation_one_generation(lattice, selectionRate, reproductionRate, colonisationRate,  exchangeRate, baseRate,
                              deathRate, cost, good_per_cooperator):
    L = len(lattice[0])
    reaction_rate = selectionRate + reproductionRate + exchangeRate + colonisationRate + deathRate
    i = 0
    ...
    while i < (L ** 2) * reaction_rate:
        ...
        reaction_single(lattice, selectionRate, reproductionRate, colonisationRate, baseRate, deathRate, cost, good_per_cooperator)
        i += 1
