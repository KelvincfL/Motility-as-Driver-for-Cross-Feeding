...
def diffusion(lattice, n_diff, defectorExchangeRate, cooperatorExchangeRate):
    L = len(lattice[0])
    i = 0
    while i < n_diff:
        ...
        if site_neighbour != lattice[site_to_update[0]][site_to_update[1]]:
            # need to include neighbour as well, currently has lower effective motility than original
            if (lattice[site_to_update[0]][site_to_update[1]] == 0 or site_neighbour == 0) and \
                    (random.random() < defectorExchangeRate / (
                            defectorExchangeRate + cooperatorExchangeRate)):  # defector
                lattice[(site_to_update[0] + position[0]) % L][(site_to_update[1] + position[1]) % L] = \
                    lattice[site_to_update[0]][site_to_update[1]]
                lattice[site_to_update[0]][site_to_update[1]] = site_neighbour
            elif (lattice[site_to_update[0]][site_to_update[1]] == 1 or site_neighbour == 1) and \
                    (random.random() > defectorExchangeRate / (
                            defectorExchangeRate + cooperatorExchangeRate)):  # cooperator
                lattice[(site_to_update[0] + position[0]) % L][(site_to_update[1] + position[1]) % L] = \
                    lattice[site_to_update[0]][site_to_update[1]]
                lattice[site_to_update[0]][site_to_update[1]] = site_neighbour
        i += 1
...
def simulation_one_generation(lattice, selectionRate, reproductionRate, colonisationRate, baseRate, cost,
                              good_per_cooperator, defectorExchangeRate, cooperatorExchangeRate):
    L = len(lattice[0])
    exchangeRate = defectorExchangeRate + cooperatorExchangeRate
    ...
    while i < (L ** 2) * reaction_rate:
        ...
        diffusion(lattice, n_diff, defectorExchangeRate, cooperatorExchangeRate)
