...
sparsity = 100
e_range = np.logspace(-5, -2, base=10, num=4, endpoint=True)
...
for i, j, lattice in gen_iter():
    for k in trange(division,leave=False):
        for l in range(iterations//division):
            chemotaxis.simulation_one_generation(lattice, selectionRate, reproductionRate, colonisationRate, baseRate, cost,
                                                 good_per_cooperator, e_range[i], bias_factor)
        density = calc_density(lattice)
        print(density)
        if density[0] == 0:
